---
layout: post
title: "Windows RPC Research Notes"
date: 2025-10-01
categories: [Research, Vulnerability Research]
tags: [RPC, Winlogon, Ghidra, WinDBG, LPE, Windows, vulnerability-hunting]
description: "Exploring Windows RPC interfaces in Winlogon.exe — reviewing security callbacks, tracing dispatch tables through dynamic and static analysis, and documenting denial-of-service research notes."
---
**Introduction:** Remote Procedure Calls (RPCs) are
...
**Research:** We will first require a way to find all
RPC Interfaces available, let's use RPCView.exe to take a look at the
...
76F226C3 is what we saw inside of RPCView.exe, It's
...
impersonation. Let's take a look at the next one:
...
The security callback seems to be doing nothing about
...
We see these functions that look like the actual RPC
...
taking actions. It's instead called through this structure.
...
This seems to be pointing to nowhere, i.e. this is
...
pointers, those might have been dynamically changed
...
We found the function. Therefore for that particular
...
Privilege Escalation (sUPE).

**A more accurate description of what we have going
on:**

The RPC Interface is the function that sets up the
RPC server through APIs like RPCRegisterServerIfEx:

> <img src="/assets/images/RPCHunt/Media/image25.png" alt="ScreenshotImg" 
> style="width:4.9375in;height:2.22917in" />
>
> The first argument is the Interface structure
...
> conditions required for the user for the RPC to happen.
...
**Let’s go through each of individual functions in
the dispatch table:**

1.  We already know through dynamic analysis, this
    one called the pointer that didn't change aka contained the error
    code.

2.  The second one was also checked and contains
    several interesting functions that can allow us to Logout other
    users, change the state of the machine etc.

3.  The third function takes us to “7ff62ccdde00”
    ...
    function named ‘WMsgNotifyHandler”

4.  Points to
    “WmsgSendReconnectionUpdateMessage”

I am starting to see a pattern here. The function
...
The first function can allow the user to make
...
It’s similar to how the IRP_DEVICE_CONTROL Function
...
WlStateMachineSetSignal which can be used for DoS.

**Conclusion:** We were able to play around with RPCs
...
**Future Work:** We will look at other applications
...
we find a viable candidate, then we can work on making a PoC.
